...
function getCartForUser(uid) {
  const items = carts[uid] || {};
  return Object.entries(items).map(([pid, qty]) => {
...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    let user = Object.values(users).find(u => u.google_sub === payload.sub);
    if (!user) {
      user = {
...
app.put('/api/profile/address', requireAuth, (req, res) => {
  const { line1 = '', line2 = '', city = '', state = '', pincode = '' } = req.body || {};
  const user = users[req.session.userId];
  if (!line1.trim() || !city.trim() || !state.trim() || !pincode.trim()) return res.status(400).json({ error: 'missing' });
...
app.post('/api/profile/phone/send-otp', requireAuth, (req, res) => {
  const { phone } = req.body || {};
  if (!phone) return res.status(400).json({ error: 'invalid' });
  const code = String(Math.floor(100000 + Math.random() * 900000));
  phoneOtps[req.session.userId] = { code, phone, expiresAt: Date.now() + 5 * 60 * 1000, attempts: 0 };
  console.log(`[DEV] OTP: ${code}`);
  res.json({ ok: true, dev_only_code: code });
});

app.post('/api/profile/phone/verify-otp', requireAuth, (req, res) => {
  const { code } = req.body || {};
  const otp = phoneOtps[req.session.userId];
  if (!otp) return res.status(400).json({ error: 'no_otp' });
  if (Date.now() > otp.expiresAt) { delete phoneOtps[req.session.userId]; return res.status(400).json({ error: 'expired' }); }
  if (otp.attempts >= 5) { delete phoneOtps[req.session.userId]; return res.status(429).json({ error: 'too_many' }); }
  if (String(code).trim() !== otp.code) { otp.attempts++; return res.status(400).json({ error: 'wrong' }); }
  const user = users[req.session.userId];
  user.phone = otp.phone;
  user.phone_verified = true;
  delete phoneOtps[req.session.userId];
  res.json({ user: publicUser(user) });
});

app.post('/api/checkout', requireAuth, (req, res) => {
  const cart = getCartForUser(req.session.userId);
  if (cart.length === 0) return res.status(400).json({ error: 'empty' });
  const subtotal = cart.reduce((s, c) => s + c.price * c.quantity, 0);
  const shipping = subtotal > 2000 ? 0 : 99;
  const orderId = orderIdCounter.val++;
  orders[orderId] = { id: orderId, userId: req.session.userId, items: cart, subtotal, shipping, total: subtotal + shipping, createdAt: new Date().toISOString() };
...
app.get('/api/admin/orders', requireAuth, (req, res) => {
  ...
  const allOrders = Object.values(orders).map(o => {
    const orderUser = users[o.userId];
    return {
      ...o,
      customerName: orderUser ? orderUser.name : 'Unknown',
      customerEmail: orderUser ? orderUser.email : 'Unknown',
      customerPhone: orderUser ? orderUser.phone : 'N/A',
      customerAddress: orderUser ? `${orderUser.address_line1}, ${orderUser.city}` : 'N/A'
    };
  });
